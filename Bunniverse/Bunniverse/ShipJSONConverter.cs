namespace Bunniverse
{
    ...
    {        
        ...
        public ShipJSONConverter(string outputPath)
        {
        ...
        {
            IEnumerable<ShipsTravel> travel = GetDataFromSql();
        ...
        private IEnumerable<ShipsTravel> GetDataFromSql()
        {
            ...
            using (var context = new BunniverseEntities())
            {
                ...
                {
                    var currentShipVists = allShipAllVisits.Where(x => x.ship.ShipId == sh.ShipId);
                    var orderedPlanets = currentShipVists.Select(x => new {x.planet.X, x.planet.Y, x.planet.Z }).ToArray();
                    ...
                    var shipTravel = new ShipsTravel()
                    {
                        ...
                        PlanetsVisited = currentShipVists.Count(),
        ...
        {
            var writer = new StreamWriter(string.Format("{0}\\{1}", outputPath, traveledData.ID));
            using (writer)
            {
